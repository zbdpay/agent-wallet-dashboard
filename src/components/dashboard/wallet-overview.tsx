  ...
  return (
    <Card className="border-zinc-300/60 bg-white/90 shadow-sm backdrop-blur">
      <CardHeader>
        ...
        <CardDescription className="text-zinc-600">Source files and current wallet identity</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-4 text-sm">
        <div className="grid gap-2 rounded-lg border border-zinc-300/70 bg-zinc-50 p-3">
          <div className="flex items-center justify-between">
            <span className="text-zinc-600">Lightning Address</span>
            <Badge variant="outline">{lightningAddress ? "Configured" : "Missing"}</Badge>
          </div>
          <p className="font-medium text-zinc-900">{lightningAddress ?? "Not found in local config"}</p>
        </div>
        ...
        <div className="grid gap-2 rounded-lg border border-zinc-300/70 bg-zinc-50 p-3">
          ...
          <p className="font-medium text-zinc-900">{hasApiKey ? "Loaded from local config (masked)" : "Not available for live balance call"}</p>
        </div>

        <div className="grid gap-2 rounded-lg border border-zinc-300/70 bg-zinc-50 p-3 text-xs text-zinc-700">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Badge className="bg-zinc-700 text-white hover:bg-zinc-700">{pendingCount} pending</Badge>
